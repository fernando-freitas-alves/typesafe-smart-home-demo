...
import { contextualizeChat } from './providers.mjs';
...
function summarizeTools(result) {
  return [{ name: 'Read Home Assistant', detail: `Read current devices and locations.`, status: 'complete' },
    ...result.stages.filter(stage => stage.kind !== 'result' && stage.kind !== 'response').map(stage => ({ name: stage.kind === 'split' ? 'Split the request' : 'Match devices and actions', provider: stage.provider, detail: stage.kind === 'split' ? stage.commands.join('\n') : stage.command, durationMs: stage.durationMs, status: 'complete' }))];
}
export class ChatService {
    ...
    if (input.op === 'send') {
      ...
      if (pending && approval.test(text)) await this.apply(data, thread, pending, input.selected, signal);
        ...
        try {
          ...
          if (resolved.clarification) thread.messages.push(message('assistant', resolved.clarification));
          else await this.preview(thread, resolved.command, signal, resolved);
        } catch (error) { thread.messages.push(message('assistant', error.message, { error: true })); }
      }
  ...
  async preview(thread, command, signal, context = {}) {
    try {
      ...
      if (context.durationMs !== undefined) tools.unshift({ name: 'Interpret follow-up', provider: 'Anthropic', detail: command, durationMs: context.durationMs, status: 'complete' });
      if (result.planId) {
      ...
      thread.messages.push(message('assistant', needsLocation ? 'Which space are you in? Choose a location below, or name the room in your next message.' : error.message,
        needsLocation ? { form: { kind: 'location' } } : { error: true }));
    }
  }
  async apply(data, thread, pending, selected, signal) {
    ...
    try {
      ...
      thread.messages.push(message('assistant', resultText.join('\n'), { tools: [{ name: 'Call Home Assistant', detail: result.calls.map(call => `${call.domain}.${call.service} → ${call.data.entity_id}`).join('\n'), status: result.error ? 'unconfirmed' : 'complete' }, { name: 'Check device states', detail: result.outcome, status: result.calls.every(call => call.observed) ? 'complete' : 'unconfirmed' }], error: Boolean(result.error) }));
    } catch (error) { pending.form.status = 'expired'; thread.messages.push(message('assistant', error.message, { error: true })); }
  }
}
