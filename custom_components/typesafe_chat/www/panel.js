...
import { renderDeviceCollections, renderReviewAction, bindDeviceControls } from './chat-components.js?v=2';
...
export class HomeChatPanel extends HTMLElement {
  ...
  actionForm(item) {
    ...
    return `<form class="approval" data-approval="${escape(item.id)}"><div class="approval-heading">${icon('tune-variant')}<strong>Review changes</strong><span>${form.actions.length} ${form.actions.length === 1 ? 'action' : 'actions'}</span></div><fieldset ${!active || this.busy ? 'disabled' : ''}><legend class="sr-only">Select the actions to apply</legend>${form.actions.map((action, index) => renderReviewAction(action, index, (form.selected || this.selections.get(item.id) || form.actions.map((_, i) => i)).includes(index), { hass: this._hass })).join('')}</fieldset>${active ? `<div class="approval-footer"><span>Apply your selection here, or reply “yes”.</span><div><button type="button" class="text-button" data-cancel ${this.busy ? 'disabled' : ''}>Cancel</button><button class="primary" type="submit" ${this.busy || this.selections.get(item.id)?.length === 0 ? 'disabled' : ''}>Apply selected</button></div></div>` : `<div class="approval-status">${icon(form.status === 'applied' ? 'check-circle-outline' : 'information-outline')}${escape(statuses[form.status] || statuses.expired)}</div>`}</form>`;
  }
  renderMessage(item) {
    if (item.role === 'context') return `<div class="context-message">${icon('map-marker-outline')}${escape(item.text)}</div>`;
    const components = item.role === 'assistant' ? renderDeviceCollections(item.components, item.id, { busy: this.busy || this.data?.thread.archived, values: this.componentValues, hass: this._hass }) : '';
