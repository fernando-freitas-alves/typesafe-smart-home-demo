...
const operations = new Set(['bootstrap', 'new', 'open', 'send', 'apply', 'cancel', 'location', 'rename', 'archive']);
export function createChatBridge({ secret, directory = './.local/chats', haUrl = 'http://127.0.0.1:8123', makeSession } = {}) {
  ...
  return createServer(async (req, res) => {
    ...
    try {
      ...
      const { actor, input, inventory, accessToken } = payload;
      ...
      if (session.busy) { session = null; return send(res, 409, { error: 'Another request is running for your account. Wait for its result.' }); }
      session.busy = true;
      ...
      const result = await session.service.handle(input, controller.signal);
      send(res, 200, result);
